document.addEventListener('DOMContentLoaded', () => {
    ...
    function speak(text) {
        ...
        const audio = new Audio(`/speech?query=${encodeURIComponent(text)}`);
        let shockwaveInterval;
        audio.addEventListener('play', () => {
            // Start shockwave animation at intervals while audio is playing
            triggerShockwave();
            shockwaveInterval = setInterval(triggerShockwave, 600); // Adjust interval as needed
        });
        function stopAllShockwavesAndResumeRecognition() {
            clearInterval(shockwaveInterval);
            stopShockwave();
            isBotSpeaking = false; // Bot finished speaking

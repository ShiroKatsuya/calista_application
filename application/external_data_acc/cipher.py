...
def get_throttling_function_name(js: str, js_url: str) -> str:
    ...
    function_patterns = [
        ...
        # Additional patterns from GitHub issue #1954
        r'a\.[a-zA-Z]\s*&&\s*\([a-z]\s*=\s*a\.get\("n"\)\)\s*&&.*?\|\|\s*([a-z]+)',
        r'\([a-z]\s*=\s*([a-zA-Z0-9$]+)(\[\d+\])?\([a-z]\)',
        r'\([a-z]\s*=\s*([a-zA-Z0-9$]+)(\[\d+\])\([a-z]\)',
    ]
    ...
    for pattern in function_patterns:
        ...
        if function_match:
            ...
            # Try to get the function name from named groups first
            try:
                if 'nfunc' in function_match.groupdict():
                    func = function_match.group('nfunc')
                elif 'n_func' in function_match.groupdict():
                    func = function_match.group('n_func') 
                elif 'func' in function_match.groupdict():
                    func = function_match.group('func')
                else:
                    # Fall back to numbered groups if named groups are not found
                    func = function_match.group(1)
            except (IndexError, KeyError):
                # If no group 1 exists, try using the whole match
                if function_match.group(0):
                    # Try to extract a function name from the match
                    possible_funcs = re.findall(r'([a-zA-Z0-9_$]{2,})', function_match.group(0))
                    if possible_funcs:
                        func = possible_funcs[-1]  # Take the last match as it's likely the function name
                    else:
                        continue  # No valid function name found, try next pattern
                else:
                    continue  # No match found, try next pattern

            # Try to get the index if it exists
            try:
                if 'idx' in function_match.groupdict():
                    idx = function_match.group('idx')
                else:
                    idx = function_match.group(2) if len(function_match.groups()) > 1 else None
            except (IndexError, KeyError):
                idx = None
            ...
            # If we don't have an index, assume the function itself is the throttling function
            return func
